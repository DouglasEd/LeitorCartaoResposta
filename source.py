...
def calcularAngulo(Cantos):
    x1,y1= cv2.boundingRect(Cantos[0])[:2]
    x2,y2= cv2.boundingRect(Cantos[1])[:2]
    dx = x1-x2
    dy = y1-y2
...
def cortarGabarito(img_path):
    ...
    for cnt in contornos_filtrados:
        ...
        proporcao = cv2.countNonZero(roi) / (roi.shape[0] * roi.shape[1])
    ...
    for cnt in contornos_filtrados:
        ...
        if proporcao > 0.3:
            ...
            questao, alternativa = 1 + (math.floor(x / tamanho_x)) + (math.floor(y / tamanho_y) * 15) , int((y - (tamanho_y * (math.floor(y / tamanho_y)))) / ((tamanho_y - 30) / 5))
